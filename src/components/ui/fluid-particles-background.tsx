...
export const FluidParticlesBackground = ({
  children,
  particleCount = 500, // Reduced count to be subtle and performant 
  noiseIntensity = 0.003,
  particleSize = { min: 0.5, max: 2 },
  speedMultiplier = 0.5, // Slow down particles
  className,
  ...
  useEffect(() => {
    ...
    // Use container dimensions so it flows within the section properly 
    const resizeCanvas = () => {
    ...
    const animate = () => {
      ...
      for (const particle of particles) {
        ...
        const opacity =
          Math.sin((particle.life / particle.maxLife) * Math.PI) * 0.25; // Fade in and out
        ...
        particle.velocity.x = Math.cos(angle) * 2 * speedMultiplier;
        particle.velocity.y = Math.sin(angle) * 2 * speedMultiplier;
  ...
  }, [particleCount, noiseIntensity, particleSize, noise, speedMultiplier]);
